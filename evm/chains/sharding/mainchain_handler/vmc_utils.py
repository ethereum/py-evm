...
viper_rlp_decoder_tx = rlp.decode(decode_hex(""), SpuriousDragonTransaction)
...
viper_rlp_decoder_addr = to_checksum_address(
    generate_contract_address(to_canonical_address(viper_rlp_decoder_tx.sender), 0)
)

sighasher_tx = rlp.decode(decode_hex(""), SpuriousDragonTransaction)
sighasher_addr = to_checksum_address(
    generate_contract_address(to_canonical_address(sighasher_tx.sender), 0)
)
...
def create_valmgr_tx(gasprice=GASPRICE):
    ...
    valmgr_addr = to_checksum_address(
        generate_contract_address(to_canonical_address(valmgr_sender_addr), 0)
    )
    _valmgr_sender_addr = to_checksum_address(valmgr_sender_addr)
...
def mk_initiating_contracts(sender_privkey, sender_starting_nonce, gasprice=GASPRICE):
    ...
    for tx in (viper_rlp_decoder_tx, sighasher_tx, get_valmgr_tx()):
        funding_tx_for_tx_sender = SpuriousDragonUnsignedTransaction(
            nonce,
            gasprice,
            500000,
            tx.sender,
...
def mk_contract_tx_obj(func_name,
    ...
    func_selector = function_abi_to_4byte_selector(func_abi)
    data = func_selector + encode_abi(arg_types, args)
    data = encode_hex(data)
    obj = {
        'from': to_checksum_address(sender_addr),
