...
# TODO: the RLPList in the new `rlp_decoder_tx_hex` doesn't work fine, so for now use the old one
...
viper_rlp_decoder_tx = pipe(
    viper_rlp_decoder_tx_hex,
    decode_hex,
    functools.partial(rlp.decode, sedes=SpuriousDragonTransaction),
)
viper_rlp_decoder_addr = pipe(
    viper_rlp_decoder_tx.sender,
    to_canonical_address,
...
def create_valmgr_tx(gasprice=GASPRICE):
    ...
    _valmgr_sender_addr = tx.sender
    _valmgr_addr = generate_contract_address(_valmgr_sender_addr, 0)
    _valmgr_tx = tx
...
def mk_contract_tx_obj(func_name,
                       ...
                       gas_price):
    """
    :param contract_addr: bytes
    :param sender_addr: bytes
    :return: the tx_obj as the input of `call` and `sendTransaction` in web3.py
    """
