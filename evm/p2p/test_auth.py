...
def test_handshake():
    ...
    # Check that the auth message generated by the initiator is what we expect.
    auth_init = initiator.create_auth_message(test_values['initiator_nonce'])
    assert len(auth_init) == len(test_values['auth_plaintext'])
    assert auth_init[65:] == test_values['auth_plaintext'][65:]  # starts with non deterministic k

    # Check that encrypting and decrypting the auth_init gets us the orig msg.
    auth_init_cipher = initiator.encrypt_auth_message(auth_init)
    assert auth_init == ecies.decrypt(auth_init_cipher, responder.privkey)

    # Check that the responder correctly decodes the auth msg.
    auth_msg_cipher = test_values['auth_ciphertext']
    initiator_ephemeral_pubkey, initiator_nonce, auth_init = responder.decode_authentication(
        auth_msg_cipher)
    ...
    auth_ack = test_values['authresp_ciphertext']
    aes_secret, mac_secret, egress_mac, ingress_mac = responder.derive_secrets(
        initiator_nonce, test_values['receiver_nonce'],
        initiator_ephemeral_pubkey, auth_init, auth_ack)
    ...
    (initiator_aes_secret,
     ...
     initiator_ingress_mac) = initiator.derive_secrets(
         ...
         responder_ephemeral_pubkey, auth_init, auth_ack)
...
def test_handshake_eip8():
    ...
    aes_secret, mac_secret, _, _ = responder.derive_secrets(
        initiator_nonce, responder_nonce, initiator_ephemeral_pubkey, auth_init,
        auth_ack_ciphertext)
    ...
    initiator_aes_secret, initiator_mac_secret, _, _ = initiator.derive_secrets(
        initiator_nonce, responder_nonce, responder_ephemeral_pubkey, auth_init,
        auth_ack_ciphertext)
