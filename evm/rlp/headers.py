...
from evm.utils.numeric import int_to_bytes32
from evm.utils.padding import pad32

from .sedes import address, hash32, int32, int256, trie_root

from evm.vm.execution_context import ExecutionContext
...
class BlockHeader(rlp.Serializable):
    ...
    def __repr__(self) -> str:
        return "<BlockHeader #{0} {1}>".format(
            self.block_number, encode_hex(self.hash)[2:10]
        )
    ...
    def create_execution_context(
        self, prev_hashes: Union[Tuple[bytes], Tuple[bytes, bytes]]
    ) -> ExecutionContext:
...
class CollationHeader(rlp.Serializable):
        ...
        if len(encoded) != self.smc_encoded_size:
            raise ValueError(
                "Encoded header size is {} instead of {} bytes".format(
                    len(encoded), self.smc_encoded_size
                )
            )
        return encoded
    ...
    def _decode_header_to_dict(cls, encoded_header: bytes) -> Iterator[Tuple[str, Any]]:
        ...
        start_indices = accumulate(
            lambda i, field: i + field[2], cls.fields_with_sizes, 0
        )
