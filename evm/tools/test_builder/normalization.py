...
from cytoolz import assoc_in, compose, concat, identity
...
from .builder_utils import deep_merge, is_cleanly_mergable
...
normalize_storage = compose(
    cytoolz.curried.keymap(normalize_int), cytoolz.curried.valmap(normalize_int)
)


normalize_state = compose(
    cytoolz.curried.keymap(to_canonical_address),
    cytoolz.curried.valmap(
        dict_normalizer(
            {
...
normalize_environment = dict_options_normalizer([normalize_main_environment])
...
normalize_transaction = dict_options_normalizer([normalize_main_transaction])
...
normalize_call_creates = eth_utils.curried.apply_formatter_to_array(
    normalize_call_create_item
)
