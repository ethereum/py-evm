...
from evm.utils.headers import compute_gas_limit
from evm.rlp.headers import BlockHeader

from .constants import FRONTIER_DIFFICULTY_ADJUSTMENT_CUTOFF
...
def compute_frontier_difficulty(parent_header, timestamp):
    ...
    if timestamp - parent_header.timestamp < FRONTIER_DIFFICULTY_ADJUSTMENT_CUTOFF:
        base_difficulty = max(parent_header.difficulty + offset, difficulty_minimum)
    else:
        base_difficulty = max(parent_header.difficulty - offset, difficulty_minimum)
    ...
    if num_bomb_periods >= 0:
        difficulty = max(base_difficulty + 2 ** num_bomb_periods, DIFFICULTY_MINIMUM)
    else:
...
def create_frontier_header_from_parent(parent_header, **header_params):
    ...
    if "gas_limit" not in header_params:
        header_params["gas_limit"] = compute_gas_limit(
            parent_header, gas_limit_floor=GENESIS_GAS_LIMIT
        )
