...
from evm.exceptions import OutOfGas, WriteProtection
...
class BaseCall(Opcode, metaclass=ABCMeta):
    ...
    def __call__(self, computation):
        ...
        if insufficient_funds or stack_too_deep:
            ...
            self.logger.debug("%s failure: %s", self.mnemonic, err_message)
...
class CallEIP150(Call):
    ...
    def compute_msg_gas(self, computation, gas, to, value):
        ...
        return compute_eip150_msg_gas(
            computation, gas, extra_gas, value, self.mnemonic, constants.GAS_CALLSTIPEND
        )
    ...
    if computation.get_gas_remaining() < extra_gas:
        ...
        raise OutOfGas(
            "Out of gas: Needed {0} - Remaining {1} - Reason: {2}".format(
                extra_gas, computation.get_gas_remaining(), mnemonic
            )
        )
...
class CallEIP161(CallEIP150):
    ...
    def compute_msg_extra_gas(self, computation, gas, to, value):
        account_is_dead = not computation.state.account_db.account_exists(
            to
        ) or computation.state.account_db.account_is_empty(to)
