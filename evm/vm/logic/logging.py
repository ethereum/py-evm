...
def log_XX(computation, topic_count):
    ...
    if not topic_count:
        ...
        topics = computation.stack_pop(
            num_items=topic_count, type_hint=constants.UINT256
        )
    else:
        topics = [
            computation.stack_pop(num_items=topic_count, type_hint=constants.UINT256)
        ]
    ...
    computation.consume_gas(total_gas_cost, reason="Log topic and data gas cost")

    computation.extend_memory(mem_start_position, size)
    log_data = computation.memory_read(mem_start_position, size)

    computation.add_log_entry(
        account=computation.msg.storage_address, topics=topics, data=log_data
    )
