...
from evm.exceptions import Halt, Revert, WriteProtection

from evm.utils.address import force_bytes_to_address, generate_contract_address
...
def selfdestruct_eip150(computation):
    ...
    if not computation.state.account_db.account_exists(beneficiary):
        computation.consume_gas(
            constants.GAS_SELFDESTRUCT_NEWACCOUNT, reason=mnemonics.SELFDESTRUCT
        )
    _selfdestruct(computation, beneficiary)
...
def selfdestruct_eip161(computation):
    ...
    is_dead = not computation.state.account_db.account_exists(
        beneficiary
...
class Create(Opcode):
    ...
    def __call__(self, computation):
        ...
        insufficient_funds = (
            computation.state.account_db.get_balance(computation.msg.storage_address)
            < value
        )
