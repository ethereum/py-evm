...
class BasePeerPool(BaseService, AsyncIterable[BasePeer]):
    ...
    async def handshake(self, remote: Node) -> BasePeer:
        ...
        try:
            (aes_secret,
             ...
             reader,
             writer
             ) = await auth.handshake(remote, self.privkey, self.cancel_token)
        ...
        peer = self.get_peer_factory().create_peer(
            remote=remote,
            connection=connection,
            inbound=False,
        )
    ...
    async def connect(self, remote: Node) -> BasePeer:
        ...
        try:
            ...
            peer = await self.wait(self.handshake(remote))
