...
@pytest.mark.parametrize("block_slot_or_hash, max_blocks, result", [
    ...
    (TEST_BRANCH[0].signed_root, 100, ()),
    (TEST_BRANCH[0].signed_root, 100, TEST_BRANCH[:1]),
    (TEST_BRANCH[0].signed_root, 100, TEST_BRANCH[:2]),
    (TEST_BRANCH[0].signed_root, 100, TEST_BRANCH),
])
...
@pytest.mark.parametrize("block_slot_or_hash, max_blocks, result", [
    ...
    (TEST_BRANCH[0].signed_root, 100, TEST_BRANCH[1:]),
    (TEST_BRANCH[0].signed_root, 100, TEST_BRANCH[::-1]),
    (TEST_BRANCH[0].signed_root, 100, (TEST_BRANCH[0], TEST_BRANCH[2])),
    (TEST_BRANCH[0].signed_root, 100, (TEST_BRANCH[0], TEST_BRANCH[2], TEST_BRANCH[1])),
    (TEST_BRANCH[0].signed_root, 100, (TEST_BRANCH[0],) + TEST_BRANCH[1::-1]),
    (TEST_BRANCH[0].signed_root, 99, TEST_BRANCH),
