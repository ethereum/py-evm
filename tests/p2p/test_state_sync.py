...
def test_state_sync():
    ...
    while requests:
        ...
        requests = scheduler.next_batch(10)
    dest_account_db = AccountDB(dest_db, account_db.root_hash)
    for addr, account_data in contents.items():
        balance, nonce, storage, code = account_data
        assert dest_account_db.get_balance(addr) == balance
        assert dest_account_db.get_nonce(addr) == nonce
        assert dest_account_db.get_storage(addr, 0) == storage
        assert dest_account_db.get_code(addr) == code
