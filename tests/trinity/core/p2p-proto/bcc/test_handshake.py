...
async def test_directly_linked_peers(request, event_loop):
    ...
    assert alice.head_slot == bob.context.chain_db.get_canonical_head().slot
    assert bob.head_slot == alice.context.chain_db.get_canonical_head().slot
...
async def test_unidirectional_handshake():
    ...
    alice_head_slot = alice_chain_db.get_canonical_head().slot
