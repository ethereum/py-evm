...
async def test_directly_linked_peers(request, event_loop, peer_class, proto):
    peer1, _ = await get_directly_linked_peers(request, event_loop, peer1_class=peer_class)
    assert isinstance(peer1.sub_proto, proto)


@pytest.mark.asyncio
async def test_les_handshake():
    peer1, peer2 = await get_directly_linked_peers_without_handshake(peer1_class=LESPeer)
