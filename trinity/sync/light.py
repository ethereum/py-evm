...
class LightChainSyncer(BaseService, PeerPoolSubscriber):
    logger = logging.getLogger("trinity.sync.light.LightChainSyncer")
    ...
    def __init__(self,
        ...
        if token is None:
            token = CancelToken('LightChainSyncer')
        else:
            token = token.chain(CancelToken('LightChainSyncer'))
    ...
    async def _handle_peer(self, peer: LESPeer) -> None:
        ...
        while True:
            try:
                ...
                break
            # We currently implement only the LES commands for retrieving data
            # (apart from Announce), and those should always come as a response
            # to requests we make so will be handled by
            # LESPeer.handle_sub_proto_msg().
    ...
    async def get_sync_start_block(self, peer: LESPeer, head_info: les.HeadInfo) -> int:
        ...
        if last_peer_announcement is None:
