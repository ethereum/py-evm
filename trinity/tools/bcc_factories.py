...
class BeaconBlockFactory(factory.Factory):
    ...
    @classmethod
    def _create(
        cls, model_class: Type[BeaconBlock], *args: Any, **kwargs: Any
    ) -> BeaconBlock:
    ...
    @to_tuple
    def create_branch(
        cls, length: int, root: BeaconBlock = None, **kwargs: Any
    ) -> Iterable[BeaconBlock]:
        if length == 0:
            return
...
class AsyncBeaconChainDBFactory(factory.Factory):
    ...
    @classmethod
    def _create(
        cls, model_class: Type[AsyncBeaconChainDB], *args: Any, **kwargs: Any
    ) -> AsyncBeaconChainDB:
        ...
        chain_db.persist_block_chain(
            blocks, BeaconBlock, (higher_slot_scoring,) * len(blocks)
        )
        return chain_db
...
class BCCPeerPoolFactory(factory.Factory):
    ...
    @classmethod
    @asynccontextmanager
    async def run_for_peer(
        cls, peer: BCCPeer, **kwargs: Any
    ) -> AsyncIterator[BCCPeerPool]:
